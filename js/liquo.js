...
(function($) {
    ...
    $.fn.liquo = function(options) {
        ...
        return this.each(function() {
            ...
            var $options = $menu.children("button[data-liquo]");
            var last = null;
            ...
            if (settings.start != "all" && $menu.children("button[data-liquo="+settings.start+"]").length > 0) {

                // Set active class on navigation
                $menu.children("button[data-liquo="+settings.start+"]").addClass("liquo-active");

                // Hide everything except active elements
                $this.find("img").not("[data-liquo="+settings.start+"]").addClass("inactive").hide(300);
                ...
                // Add active to all
                $menu.children("button[data-liquo=all]").addClass("liquo-active");
            ...
            if (settings.random) {
                ...
                $this.children("img").sort(function(a,b) {
            ...
            $($options).click(function(e) {
                ...
                if (category == "all") {
                    ...
                    // Show all elements
                    $this.find("img").show(300).removeClass("inactive");

                    // Set last to null
                    last = null;

                } else if ($this.find("img[data-liquo="+category+"]").length > 0) {

                    // If first category clicked
                    if (last == null) {

                        // Hide everything except current category
                        $this.find("img").not("[data-liquo="+category+"]").addClass("inactive").hide(300);

                    } else if (category != last) {

                        // Hide last category elements
                        $this.find("img[data-liquo="+last+"]").addClass("inactive").fadeOut(300, function() {

                            // Show new category elements
                            $this.find("img[data-liquo="+category+"]").removeClass("inactive").stop().show(300);
